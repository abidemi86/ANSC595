...
meta<-read_q2metadata("metadataproject.txt")
line 67 didnt work
...
#alpha_diversity = merge(x=faith_pd, y=evenness, by.x = "SampleID", by.y = "SampleID")# original code
#alpha_diversity = merge(alpha_diversity, observed_features, by.x = "SampleID", by.y = "SampleID")#couldnt use this either
alpha_diversity = merge(x=faith_pd, y=observed_features, by.x = "SampleID", by.y = "SampleID")
...
qp.faith.time = glm(faith_pd ~ delivery_mode.ord, data=meta, family="quasipoisson")
...
#Plot
plot(log(faith_pd) ~ delivery_mode.ord, data=meta, ylab="ln(Faith Phylo. Diversity)")
plot(log(faith_pd) ~ delivery_mode.ord, data=meta, ylab="ln(Faith Phylo. Diversity)") + abline(qp.faith.time)
